...
$j(document).ready(function () {
    "use strict";
    header_height = $j('header').height() + 1;
    dropDownMenu();
    dropDownWpml();
...
function startAnimation() {
    ...
    $j('.header_right_widget').css('transition-delay', 0.5 + main_nav_num * 0.1 + 0.1 + 's');
    setTimeout(function () {
        ...
        setTimeout(function () {
            ...
            setTimeout(function () {
                ...
                if ($j('.animation_content').length) {
                    $j('.animation_content .container_inner, .animation_content .full_width_inner').css('-webkit-transform', 'none');
                    initToCounter();
                    initCounter();
                    initListAnimation();
                    initServices();
        ...
        if ($j(menu_items[i]).find('.second').length > 0) {
            ...
            $j(this).mouseenter(function () {
                ...
                $j(this).find('.second').stop().animate({
                    'height': $j(this).data('original_height'),
                    'margin-top': '0px',
                    opacity: 1
                }, 400, function () {
...
function socialShare() {
    ...
    if ($j(menu_item).length > 0) {
        menu_item.each(function (i) {
            ...
            var ul_width = maxWidthImage + maxWidthText + 5;
            $j(menu_item[i]).parent().mouseenter(function () {
                $j(menu_item[i]).find('ul').css({
                    'width': ul_width
                });
                $j('.share_text').css({
                    'width': maxWidthText - 10
                });
                $j(menu_item[i]).css({
                    'visibility': 'visible',
                    'overflow': 'visible',
                    ...
                    'display': 'block',
                    'margin-top': '40px',
                    'margin-left': -ul_width / 2
                });
                $j(menu_item[i]).stop().animate({
                    'margin-top': '0px',
                    'opacity': '1'
                }, 400)
            }).mouseleave(function () {
...
function initBlog() {
    ...
    if ($j('.blog_holder_v3').length) {
        ...
        $container.isotope({
            ...
            resizable: false,
            masonry: {
                columnWidth: $j('.blog_holder_v3').width() / $cols
            }
        });
        $j(window).resize(function () {
            if ($j('.container_inner').width() === 420) {
...
function checkLogo() {
    ...
    $j('.logo a img.normal').css('display', 'block')
}
...
function addPlaceholderSearchWidget() {
    "use strict";
    $j('.header_right_widget #searchform input:text, .widget.widget_search form input:text, footer .footer_top_inner #searchform input:text').each(function (i, el) {
...
function initSocialWidget() {
    "use strict";
    if ($j(window).width() > 768) {
        $j('#social_icons_widget .social_icons_widget_inner').css({
            'height': $j('#social_icons_widget .social_icons_widget_inner').height(),
            'visibility': 'visible'
        })
    }
}
    ...
    $j('.more_facts_holder').each(function () {
        ...
        $j(this).find('.more_facts_button').click(function () {
            if (!$j(this).hasClass('facts_opened')) {
                $j(this).parent().parent().find('.more_facts_outer').css('display', 'block').stop().animate({
                    'height': height + 30
                }, speed);
